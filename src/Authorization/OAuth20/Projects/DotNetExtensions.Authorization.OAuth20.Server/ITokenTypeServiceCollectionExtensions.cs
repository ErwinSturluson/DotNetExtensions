...
public static class ITokenTypeServiceCollectionExtensions
{
    ...
    public static IServiceCollection SetOAuth20TokenTypesFromConfiguration(this IServiceCollection services)
    {
        ...
        if (options.Tokens?.TokenTypes is not null && options.Tokens.TokenTypes.Any())
        {
            foreach (var tokenTypeOptions in options.Tokens.TokenTypes)
            {
                if (tokenTypeOptions.Builder is null)
                {
                    continue;
                }

                if (tokenTypeOptions.Builder.Implementation is null)
                {
                    continue;
                }
                ...
                {
                    if (!TryGetType(services, tokenTypeOptions.Name, out abstractionType))
                    {
                        continue;
                    }
                }
                ...
                if (!TryGetType(tokenTypeOptions.Builder.Implementation.AssemblyName, tokenTypeOptions.Builder.Implementation.TypeName, out Type? implementationType))
                {
                    continue;
                }
